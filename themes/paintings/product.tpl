        ...
        <div class="primary_block row">
            ...
            <div class="product-pagination">
                ...
                <div class="previous_link">
                    {if $prev_product}
                        <a title="{$prev_product.name}" href="{$link->getProductLink($prev_product.id_product, $prev_product.link_rewrite)}">« {l s='Previous Product'}</a>
                    {/if}
                </div>
                <div class="next_link">
                    {if $next_product}
                        <a title="{$next_product.name}" href="{$link->getProductLink($next_product.id_product, $next_product.link_rewrite)}">{l s='Next Product'} »</a>
                    {/if}
                </div>
                <div class="related_artists">
                    {if !empty($related_artists)}
                        <a id="related_button" href="#">{l s='Show related artists'} »</a>
                        <ul id="related_list">
                            {foreach from=$related_artists item=artist}
                                <li>
                                    <a href="{$link->getCategoryLink($artist.id_category, $artist.link_rewrite)|escape:'html':'UTF-8'}" title="{$artist.name|escape:'html':'UTF-8'}">
                                        {$artist.name}
                                    </a>
                                </li>
                            {/foreach}
            ...
            <div class="pb-left-column col-xs-12 col-sm-4 col-md-5 col-md-offset-2">
                                        ...
                                        <li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
                                            <a{if $jqZoomEnabled && $have_image && !$content_only} href="javascript:void(0);" rel="{literal}{{/literal}gallery: 'gal1', smallimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}',largeimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}'{literal}}{/literal}"{else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}" data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if}
                                                ...
                                                <img class="img-responsive" id="thumb_{$image.id_image}"
                                                     ...
                                                     height="{$cartSize.height}" width="{$cartSize.width}"
                                                     itemprop="image"/>
                                            </a>
                                        </li>
                                    {/foreach}
                ...
                {if isset($meta_keywords) && $meta_keywords}
                    <div class="product-keywords">
                        <span>Keywords - </span>
                        {assign var="keywords_array" value=","|explode:$meta_keywords}
                        {foreach from=$keywords_array key=number item=keyword}
                            {if $keywords_array|@count != $number+1}
                                <a href="{$base_dir}search?controller=search&search_query={$keyword|replace:' ':''}">{$keyword|replace:' ':''}</a>,
                            {else}
                                <a href="{$base_dir}search?controller=search&search_query={$keyword|replace:' ':''}">{$keyword|replace:' ':''}</a>
                            {/if}
                        {/foreach}
            ...
            <div class="pb-right-column col-xs-12 col-sm-4 col-md-3">
                                ...
                                {if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
                                    ...
                                    <div>
                                        <p class="our_price_display" itemprop="offers" itemscope
                                           itemtype="http://schema.org/Offer">{strip}
                                    ...
                                    {/if}
                                        ...
                                        <p id="old_price"{if (!$product->specificPrice || !$product->specificPrice.reduction) && $group_reduction == 0} class="hidden"{/if}>{strip}
                                                {if $priceDisplay >= 0 && $priceDisplay <= 2}
                                                    ...
                                                    <span id="old_price_display">{if $productPriceWithoutReduction > $productPrice}
                                                            <span class="price">{convertPrice price=$productPriceWithoutReduction}</span>{if $tax_enabled && $display_tax_label == 1} {if $priceDisplay == 1}{l s='tax excl.'}{else}{l s='tax incl.'}{/if}{/if}{/if}</span>
                                                {/if}
                                            {/strip}</p>
                                        {if $priceDisplay == 2}
                                            <br/>
                ...
                <div class="product-shiping">
                    {include file="$tpl_dir./product-shiping.tpl"}
                    {*{var_dump($category)}*}
                    {l s='This %s Oil Painting reproduction masterpiece is hand painted by a real artist. No digital or printing techniques are used. You are commissioning a real painting.' sprintf=$category->name}

                </div>
            </div>
            <!-- end pb-right-column-->
        </div>
        {include file="$tpl_dir./free-shiping.tpl"}
        <!-- end primary_block -->
        {if !$content_only}
